...
class MaskedGenerator(nn.Module):
    ...
    def forward(self, m, z, c):
        # print(z.view(z.size(0),z.size(1)).shape)
...
class EncM(nn.Module):
    def __init__(self, sizex=128, nIn=3, nMasks=2, nRes=5, nf=128, temperature=1):
        ...
        self.psp = nn.ModuleList([nn.Sequential(nn.AvgPool2d(sizex),
                                                nn.Conv2d(nf,1,1),
                                                nn.Upsample(size=sizex, mode='bilinear', align_corners=True)),
    ...
    def forward(self, x):
        x = self.cnn(x)
        m = self.out(torch.cat([x,] + [pnet(x) for pnet in self.psp], 1))
        if self.nMasks == 2:
...
class GenX(nn.Module):
    def __init__(self, sizex=128, nOut=3, nc=8, nf=64, nMasks=2, selfAtt=False):
        ...
        self.net = nn.ModuleList([MaskedGenerator(nf=nf, nOut=nOut, nc=nc, selfAtt=selfAtt) for k in range(nMasks)])
        self.nMasks = nMasks
...
class RecZ(nn.Module):
    def __init__(self, sizex=128, nIn=3, nc=5, nf=64, nMasks=2):
        super(RecZ, self).__init__()
        if sizex == 128:
            self.net = Encoder128(nIn=nIn, nf=nf, nOut=nc*nMasks)
        # elif sizex == 64:
        #     self.net = Encoder64(nIn=nIn, nf=nf, nOut=nc*nMasks)
        self.nc = nc
        self.nMasks = nMasks
    def forward(self, x):
        c = self.net(x)
